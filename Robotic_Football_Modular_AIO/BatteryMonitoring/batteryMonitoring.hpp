...
BatteryLevel getBatteryLevelFromVoltage(uint16_t voltageMilliVolts) {
    ...
    if (voltageMilliVolts > LOW_THRESHHOLD)     {return BATTERY_LOW;}
...
uint16_t getBatteryVoltageFromDivider() {
  ...
  return (3700*5.0*analogRead(A15))/1024;
}

void setLedsBasedOnBatteryLevel (PS3BT& PS3) {
	
	static unsigned long lastRunTime = millis();
	if (millis() > lastRunTime + TIME_BETWEEN_RUNS) { 
		...
		if (batteryLevel == BATTERY_LOW) {PS3.setLedOn (LED2); PS3.setLedOff(LED3);}
